...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[latent/.append style={minimum size=1.0cm}]
        ...
        \node[latent, below=of o] (a) {$\boldsymbol{a}^i$};
        \node[latent, below=of a] (s) {$\boldsymbol{s}$};
        \node[obs, right=of a] (a1) {$\boldsymbol{a}^{-i}$};
...
\subsection{PR2 with Balanced-Q}
...
\begin{figure}[ht]
    ...
    \end{minipage}%
    \begin{minipage}[t]{0.5\linewidth}
    \captionof{figure}{Graphical model of splitted graphical model for opponent model's policy. } 
    % \label{fig:}
    \end{minipage}
\end{figure}
