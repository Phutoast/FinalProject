...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[latent/.append style={minimum size=1.0cm}]
        ...
        \node[latent, below=of o] (a) {$\boldsymbol{a}^i$};
        \node[latent, below=of a] (s) {$\boldsymbol{s}$};
        \node[obs, below=of s] (a1) {$\boldsymbol{a}^{-i}$};
...
This carves the way for us by defining $\beta_{op}(\boldsymbol{s}, \boldsymbol{a}^i, \boldsymbol{a}^{-i})$ to be varied according to all agents actions and state. \Phu{VERY VERY WEIRD INITIAL IDEA. This would be a disaster for defining the normalizing factor ?}